...
const postMovieByID = async(req, res) => {
  ...
  const { data } = await axios(`https://api.themoviedb.org/3/movie/${movieId}?api_key=${process.env.API_KEY}&language=en-US`) 
...
const getPopularMovies = async(req, res) => {
  ...
  const { data } = await axios(`https://api.themoviedb.org/3/movie/popular?api_key=${process.env.API_KEY}&language=en-US&page=${page}&region=US`)
  const { data: { images: { base_url } } } = await axios(`https://api.themoviedb.org/3/configuration?api_key=${process.env.API_KEY}`)
...
const getUpcomingMovies = async(req, res) => {
  const { page } = req.params
   ...
   //  getting array of movies from the search results & image config from API
  const { data } = await axios(`https://api.themoviedb.org/3/movie/upcoming?api_key=${process.env.API_KEY}&language=en-US&page=${page}`)
  const { data: { images: { base_url } } } = await axios(`https://api.themoviedb.org/3/configuration?api_key=${process.env.API_KEY}`)
